...
// getsocketopt的每个可能得返回值，val类型中都有一个成员
union val {
  int				i_val;
  long				l_val;
...
struct sock_opts {
  ...
  int		opt_level;
  int		opt_name;
  char   *(*opt_val_str)(union val *, int); // opt_val_str是指向用于4个选项值输出函数中的某一个的指针。
} sock_opts[] = {
...
main(int argc, char **argv)
{
	int					fd;
	socklen_t			len;
	struct sock_opts	*ptr;   // 指向sock_opts[]
...
{
/* *INDENT-OFF* */
	if (len != sizeof(int)) // getsockopt返回值的大小是否是期望的大小
